...
<template>
  <div class="w-full h-auto">
    <ul
      class="pattern-list"
      :style="{ gridTemplateColumns: `repeat(${data.rowCount},1fr)` }"
    >
      <li
        v-for="item in data.rowCount * 7"
        :key="item"
        class="w-5 h-5"
        :style="{
          backgroundColor: data.patternList.includes(item)
            ? data.patternColor
            : data.cardColor,
        }"
        @click.stop="(event) => updatePatternList(event, item)"
      />
    </ul>
  </div>
</template>
