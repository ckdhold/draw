...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    build: {
      ...
      minify:
        mode === "file" ? false : mode === "prebuild" ? "esbuild" : "terser", // file 模式不压缩以减少内存占用
      ...(mode === "file"
        ? {}
        : {
            terserOptions: {
      ...
      rollupOptions: {
        output: {
          chunkFileNames:
            mode === "file"
              ? "js/[name]-[hash].js"
              : `js/${chunkName}-[hash].js`, // 引入文件名的名称
          entryFileNames:
            mode === "file"
              ? "js/[name]-[hash].js"
              : `js/${chunkName}-[hash].js`, // 包的入口文件名称
          assetFileNames:
            mode === "file"
              ? "[ext]/[name]-[hash].[ext]"
              : `[ext]/${chunkName}-[hash].[ext]`, // 资源文件像 字体，图片等
          manualChunks:
            mode === "file"
              ? undefined
